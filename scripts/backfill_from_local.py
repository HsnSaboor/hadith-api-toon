...
I360_MAP = {
    'أ': ' ', 'إ': 'آ', 'ځ': 'پ', '﷽': 'ﷺ', 'ل': 'ف', 'ش': 'r', 'و': 'm', 'ت': 'a',
    'ۄ': 'h', 'ە': 'e', 'ح': 't', 'ہ': 'h', 'ڬ': 'k', 'ۏ': 'y', 'ؼ': 'a', 'ه': 'l',
    'ز': 'd', 'ً': 'o', 'ى': 'n', 'د': 'j', 'ض': 's', 'غ': 't', 'ث': 'b', 'م': 'q',
    'ڲ': 'g', 'ۗ': '.', 'ذ': 'h', 'ظ': 's', 'ط': 'sh', 'ؽ': 'gh', 'ع': 'z', 'ډ': 'ch',
    'ص': 'z', 'ڽ': 'n', '+': '(', ',': ')', '“': '"', '”': '"', '‛': '"', '$': '!',
    '=': ':', 'ة': 't', 'ؖ': 'RA', 'ر': 'kh', '؏': ',', 'س': 'z', 'ػ': 'z',
    'ڔ': 'r', 'ڋ': 'd', 'ټ': 't', 'خ': 's', 'ٔ': '', 'ٳ': '', 'آ': 'a', 'ۆ': 'o',
    '٪': '%', 'ي': 'y'
}

# The actual mapping for normal Urdu Urdu characters
I360_URDU_MAP = {
    'أ': ' ', 'إ': 'آ', 'ځ': 'پ', '﷽': 'ﷺ', 'ل': 'ف', 'ش': 'ر', 'و': 'م', 'ت': 'ا',
    'ۄ': 'ہ', 'ە': 'ے', 'ح': 'ت', 'ہ': 'ھ', 'ڬ': 'ک', 'ۏ': 'ی', 'ؼ': 'ع', 'ه': 'ل',
    'ز': 'د', 'ً': 'و', 'ى': 'ن', 'د': 'ج', 'ض': 'س', 'غ': 'ط', 'ث': 'ب', 'م': 'ق',
...
MARKDOWN_PLACEHOLDERS = ["### अनुवाद", "হাদिस নং", "অনুবাদ", "হাদীس নম্বর"]
...
def main():
    ...
    print(f"Starting optimized local backfill matching on {len(batch_files)} batches...")
    ...
    for bf in batch_files:
        ...
        for t in tasks:
            total_tasks += 1
            translation = find_translation_with_roman(t)
            if translation:
                results.append((t["hadithnumber"], translation))
                total_matched += 1
            else:
        ...
        print(f"Batch {batch_num}: matched {len(results)}/{len(tasks)} tasks.")

    print(f"\nLocal matching backfill completed!")
    print(f"Matched hadiths: {total_matched}/{total_tasks} ({total_matched/total_tasks*100:.2f}%)")
    print(f"Remaining unmatched: {len(missing_tasks)}")
    
    if missing_tasks:
        missing_path = TASKS_DIR / "unmatched_tasks.json"
        with open(missing_path, "w", encoding="utf-8") as f:
            json.dump(missing_tasks, f, ensure_ascii=False, indent=2)
        print(f"Saved unmatched tasks to {missing_path.name}")
