...
BAYHAQI_INDEX = f"{BASE_URL}/hadees-name/bayhaqi/0"
OUTPUT_DIR = "/home/saboor/code/hadith-api-toon/sunnah.com-download/bayhaqi"
...
def clean_text(text):
    ...
    for l in lines:
        if not l.strip():
            continue
        # Remove the injected site metadata at the bottom (e.g., "سنن الکبری بیہقی حدیث: 123")
        if re.search(r'(مسند|سنن|صحیح|جامع|حدیث).*?:', l):
            # If the line is short and contains this, it's metadata. 
            # If it's a long line, it might be actual text. But usually it's at the end.
            if len(l) < 150:
                continue
...
def main():
    print("Starting Sunan Al Kubra Bayhaqi Scraper from al-hadees.com...")
    
    soup = get_soup(BAYHAQI_INDEX)
    if not soup:
    ...
    for a in soup.find_all('a', href=True):
        if '/hadees-subjects/bayhaqi/' in a['href']:
            ...
            while True:
                ...
                for num_div in number_divs:
                    ...
                    if card_footer:
                        ...
                        if ar_ta:
                            ar_text = clean_text(ar_ta.text)
                            
                        ur_ta = card_footer.find('textarea', id=re.compile(r'content-urd-'))
                        if ur_ta:
                            ur_text = clean_text(ur_ta.text)
                            
                    ar_item = {
                        "hadithnumber": h_num,
                        "narrator": "",
                        "status": status,
                        "reference": f"Sunan Al Kubra Bayhaqi {h_num}",
                        "in_book_reference": "",
